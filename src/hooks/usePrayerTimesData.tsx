...
export const usePrayerTimesData = () => {
  ...
  const [tomorrowDetailedTimes, setTomorrowDetailedTimes] = useState<DetailedPrayerTime | null>(null);
  ...
  const loadData = useCallback(async () => {
    if (dataLoadingRef.current || !selectedMosque) {
      console.log("Already loading data or no mosque selected, skipping");
      return;
    }
    ...
    try {
      ...
      setIsLoading(true);
      console.log(`Loading prayer times for mosque: ${selectedMosque}...`);

      // Load standard prayer times (which calculates isActive/isNext based on current time)
      const times = await fetchPrayerTimes(selectedMosque);
      setPrayerTimes(times);

      // Load detailed prayer times for the current day AND tomorrow
      const allTimes = await fetchAllPrayerTimes(selectedMosque);
      ...
      const todayTimes = allTimes.find(t => t.date === todayString);
      if (todayTimes) {
        setDetailedTimes(todayTimes);
      }

      const tomorrowTimes = allTimes.find(t => t.date === tomorrowString);
      if (tomorrowTimes) {
        setTomorrowDetailedTimes(tomorrowTimes);
      }
  ...
  return { prayerTimes, detailedTimes, tomorrowDetailedTimes, isLoading, loadData };
};
